...
\subsection{Use case}
Il diagramma degli Use Case (Figura \ref{fig:use_case_diagram}) illustra le funzionalità principali del sistema AgroManager e le interazioni con l'attore. È stato identificato un singolo attore, il quale ha accesso a tutte le funzionalità.

Le funzionalità principali, identificate come "User Goal", sono:
\begin{itemize}
	\item \textbf{Gestire Fornitori:} Inserimento, modifica, rimozione e filtro dei fornitori con validazioni complete.
	...
	\item \textbf{Elaborare Dati e Report:} Analisi della produttività tramite strategie specializzate e generazione di report con salvataggio in formato TXT.
...
{Le informazioni sui fornitori sono aggiornate nel sistema secondo le operazioni richieste}
{
	\begin{enumerate}[nosep]
		\item L'utente accede alla sezione "Fornitori" dal menu principale
...
}
{
	...
	\begin{itemize}[nosep]
		\item[\textbf{2a.1}] Il sistema mostra un messaggio di errore "Sistema non operativo"
...
{Utente}
{L'utente si trova nella sezione Fornitori e il sistema è operativo}
{Un nuovo fornitore è stato aggiunto al sistema e l'elenco fornitori è aggiornato}
{
	\begin{enumerate}[nosep]
		...
		\item Il sistema apre il dialog di inserimento nuovo fornitore
...
}
{
	...
	\begin{itemize}[nosep]
		\item[\textbf{7a.1}] Il sistema evidenzia i campi con errori (nome, indirizzo o telefono vuoti, email non valida se inserita, telefono troppo corto)
	...
	\begin{itemize}[nosep]
		...
		\item[\textbf{7b.2}] Il sistema mostra messaggio di errore "Fornitore già esistente"
...
{Utente}
{L'utente si trova nella sezione Fornitori, esiste almeno un fornitore nel sistema, e l'utente ha selezionato un fornitore dall'elenco}
{Le informazioni del fornitore selezionato sono aggiornate e l'elenco fornitori riflette le modifiche}
{
	\begin{enumerate}[nosep]
		...
		\item Il sistema apre il dialog di modifica precompilato con i dati attuali del fornitore
...
{Il fornitore selezionato è rimosso permanentemente dal sistema e l'elenco fornitori riflette l'avvenuta rimozione}
{
	\begin{enumerate}[nosep]
		...
		\item Il sistema mostra un dialog modale di conferma (es. "Sei sicuro di voler eliminare questo fornitore?")
...
}
{
	...
	\begin{itemize}[nosep]
		...
		\item[\textbf{5a.2}] Il sistema mostra un messaggio di errore (es. "Impossibile eliminare, il fornitore è utilizzato")
...
{Utente}
{L'utente si trova nella sezione Fornitori ed esistono fornitori nel sistema da poter filtrare}
{L'elenco dei fornitori mostra solo i record che corrispondono ai criteri di ricerca e i filtri applicati sono resi visibili all'utente}
{
	\begin{enumerate}[nosep]
		...
		\item Il sistema mostra i filtri attivi (es. in appositi "tag" o mantenendo il testo nei campi)
...
}
{
	...
	\begin{itemize}[nosep]
		\item[\textbf{3a.1}] Il sistema mostra un messaggio (es. "Nessun fornitore trovato per i criteri selezionati")
		\item[\textbf{3a.2}] L'elenco dei fornitori viene mostrato vuoto
		\item[\textbf{3a.3}] L'utente può modificare i criteri di ricerca e riprovare
	\end{itemize}
}
\newpage
...
{
	\begin{enumerate}[nosep]
		\item L'utente accede alla sezione "Piantagioni" dal menu principale
		...
		\item L'utente può scegliere una delle seguenti operazioni:
		      \begin{itemize}[nosep]
			      ...
			      \item Cambiare lo stato di una piantagione (UC2.4)
...
{Utente}
{L'utente si trova nella sezione Piantagioni, esiste almeno una piantagione nel sistema, e l'utente ha selezionato una piantagione dall'elenco}
{Lo stato della piantagione è aggiornato con il nuovo stato del ciclo di vita, l'elenco piantagioni riflette il cambiamento, e la data di cambio stato è registrata nel sistema}
{
	\begin{enumerate}[nosep]
		...
		\item Il sistema apre il dialog di cambio stato
...
}
{
	...
	\begin{itemize}[nosep]
		\item[\textbf{4a.1}] Il sistema mostra solo gli stati raggiungibili dal stato attuale secondo le regole del ciclo di vita
		\item[\textbf{4a.2}] L'utente può scegliere solo tra: stato successivo, sospensione o rimozione
	\end{itemize}
...
{
	\begin{enumerate}[nosep]
		\item L'utente accede alla sezione "Analisi \& Report" dal menu principale
		...
		\item L'utente può scegliere una delle seguenti operazioni:
		      \begin{itemize}[nosep]
			      ...
			      \item Visualizzare analisi dati con strategy pattern
			      \item Esportare risultati in formato TXT
		      \end{itemize}
		\item Il sistema elabora i dati secondo l'opzione selezionata
		\item Il sistema visualizza i risultati dell'elaborazione
		\item L'utente può salvare o esportare i risultati
	\end{enumerate}
}
{
...
{Utente}
{Il sistema è avviato e l'utente si trova nella schermata principale dell'applicazione}
{La dashboard mostra informazioni aggiornate e l'utente ha una vista d'insieme dello stato del sistema}
{
	\begin{enumerate}[nosep]
		...
		\item Il sistema visualizza la dashboard con le seguenti informazioni:
		      ...
		      \end{itemize}
		\item L'utente può navigare verso sezioni specifiche cliccando sui widget della dashboard o utilizzare le azioni rapide
	\end{enumerate}
}
{
	\textbf{2a.} Sistema non operativo (database non accessibile):
	\begin{itemize}[nosep]
		\item[\textbf{2a.1}] La dashboard mostra lo stato offline con indicatore e messaggio di errore
		\item[\textbf{2a.2}] Le azioni rapide vengono disabilitate e mostrato messaggio "Connetti il database per accedere alle funzionalità"
...
\begin{itemize}
	...
	\item \textbf{Contesto del Progetto}:
	      Sto sviluppando un'applicazione JavaFX chiamata \textbf{AgroManager} per la gestione agricola seguendo il pattern architetturale \textbf{MVC (Model-View-Controller)}. Il progetto utilizza:
	      \begin{itemize}
		      \item \textbf{JavaFX} per l'interfaccia utente
		      \item Pattern \textbf{MVC} con separazione netta delle responsabilità
		      \item \textbf{DAO Pattern} per l'accesso ai dati
...
\end{lstlisting}
	      ...
	      \begin{itemize}
		      ...
		      \item \textit{Funzionalità Richieste:}
		            \begin{itemize}
			            ...
			            \item Messaggi di feedback per operazioni CRUD
		            \end{itemize}
	...
	\item \textbf{Pattern di Integrazione MVC}:
...
\end{lstlisting}
	...
	\item \textbf{Richiesta Specifica}:
	      ...
	      \begin{itemize}
		      ...
		      \item \texttt{FornitoreDialog.java} - Dialog modale per CRUD
		      \item Eventuali aggiornamenti \texttt{CSS} per stili specifici non presenti
	      \end{itemize}

	      Rispetta rigorosamente:
	      \begin{itemize}
		      \item Pattern MVC con separazione delle responsabilità
...
\begin{figure}[h]
	...
	\caption{Mockup della vista "Gestione Zone". Illustra l'interfaccia CRUD (Create, Read, Update, Delete) standard, composta da un pannello di "Ricerca", i pulsanti di azione (Nuova Zona, Modifica, ecc.) e la tabella "Elenco Zone".}
...
\begin{figure}[h]
	...
	\caption{Mockup della vista "Gestione Fornitori". Illustra l'interfaccia CRUD (Create, Read, Update, Delete) standard, composta da un pannello di "Ricerca", i pulsanti di azione (Nuovo Fornitore, Modifica, ecc.) e la tabella "Elenco Fornitori".}
...
\begin{figure}[h]
	...
	\caption{Mockup della vista "Gestione Piante". Illustra l'interfaccia CRUD (Create, Read, Update, Delete) standard, composta da un pannello di "Ricerca", i pulsanti di azione (Nuova Pianta, Modifica, ecc.) e la tabella "Elenco Piante".}
...
\begin{figure}[h]
	...
	\caption{Mockup della vista "Gestione Piantagioni". Illustra l'interfaccia CRUD (Create, Read, Update, Delete) standard, composta da un pannello di "Ricerca", i pulsanti di azione (Nuova Piantagione, Modifica, ecc.) e la tabella "Elenco Piantagioni".}
...
\begin{itemize}
	\item La \textbf{Dashboard} agisce come "hub" centrale, da cui l'utente può accedere a tutte le sezioni principali ("spokes").
	\item Ciascuna sezione gestionale (es. "Gestione Zone", "Gestione Piantagioni") segue un pattern di interazione coerente, permettendo all'utente di tornare facilmente alla Dashboard.
	\item Le operazioni CRUD (come "new/edit") vengono gestite tramite finestre di dialogo modali (es. "Zona Dialog"), che al termine ("save/cancel") riportano l'utente alla vista gestionale di provenienza.
\end{itemize}
