\section{Test}
L'infrastruttura di test del progetto è stata sviluppata utilizzando \textbf{JUnit 5} per garantire l'affidabilità, la correttezza e la manutenibilità del codice. L'approccio segue una strategia multi-livello, suddividendo i test in tre categorie principali che validano l'architettura implementata:

\begin{itemize}
	\item \textbf{Test Strutturali (Unit Test):} Focalizzati sulla verifica in \textbf{isolamento} dei singoli componenti (classi del \hyperref[fig:domain_model]{Domain Model} e logica di validazione dei \hyperref[fig:business_logic_service]{Service}), utilizzando \textit{mock object} per simulare le dipendenze esterne (come i \hyperref[sec:dao_pattern]{DAO}).

	\item \textbf{Test di Integrazione (ORM):} Mirati a validare la corretta comunicazione tra l'applicazione e il database PostgreSQL. Verificano l'intero ciclo di vita \textbf{CRUD} (Create, Read, Update, Delete) e la correttezza delle query SQL.
...
\begin{lstlisting}[
    language=Java,
    caption={Definizione di un \textbf{Mock DAO} interno alla classe di test per isolare il service.},
...
Questi test validano l'intero strato \texttt{\hyperref[fig:orm]{ORM}}, assicurando che le classi \hyperref[sec:dao_pattern]{DAO} interagiscano correttamente con il database PostgreSQL. Testano la correttezza delle query SQL, la gestione delle \texttt{SQLException} e il mapping tra \texttt{ResultSet} e oggetti del dominio, verificando che l'implementazione sia coerente con lo \hyperref[fig:er_diagram]{schema E-R del database}.
Per garantire che i test siano eseguibili più volte senza alterare lo stato del sistema, viene utilizzata una strategia di \textit{setup} e \textit{teardown}:
\begin{itemize}
...
\begin{lstlisting}[
    language=Java,
    caption={Test di integrazione per il ciclo \textbf{CRUD} completo su PiantaDAO.},
    label={lst:crud_test}
]
@BeforeEach
...
I test funzionali verificano una \textit{feature} completa (come un \hyperref[UC1]{caso d'uso}) eseguendo il codice attraverso più layer. A differenza dei test unitari, questi test non usano mock ma interagiscono con i servizi reali e il database di test per validare il risultato finale.

Il test \texttt{ReportServiceTest} è un esempio di questa strategia. Verifica che la generazione dei report (un'operazione complessa che richiede l'esecuzione di \texttt{\hyperref[sec:strategy_pattern]{Strategy}} e l'accesso ai dati tramite il \texttt{RaccoltoService}) produca un risultato corretto.
