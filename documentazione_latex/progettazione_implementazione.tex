...
\begin{itemize}
	...
	\item \textbf{Controller:} Agisce da mediatore tra la \texttt{View} e i layer sottostanti. Riceve gli eventi dall'interfaccia utente e invoca le operazioni appropriate (parte dell'\hyperref[sec:mvc_pattern]{architettura MVC}).
	...
	\item \textbf{Strategy:} Contiene le implementazioni di algoritmi specifici (es. le strategie di calcolo per i report), seguendo lo \hyperref[sec:strategy_pattern]{Strategy Pattern}.

	\item \textbf{ORM (Object-Relational Mapping):} Gestisce la mappatura e la comunicazione tra le classi del \texttt{DomainModel} e le tabelle del database (tramite i \hyperref[sec:dao_pattern]{DAO}).
...
\subsubsection{DomainModel}\label{sec:domain_model}
...
\newpage
\subsubsection{ORM}\label{sec:orm}
Il package ORM implementa il \hyperref[sec:dao_pattern]{Data Access Object (DAO) pattern} per l'accesso ai dati. Come visibile in Figura~\ref{fig:orm}, la sua struttura è composta da:
\begin{itemize}
	\item Una classe astratta BaseDAO che implementa il \hyperref[sec:template_method]{Template Method Pattern} per le operazioni CRUD comuni
...
\newpage
\subsubsection{BusinessLogic}\label{sec:business_logic}
...
\newpage
\subsubsection{BusinessLogic - Service}\label{sec:business_logic_service}
...
\newpage
\subsubsection{BusinessLogic - Strategy}\label{sec:business_logic_strategy}
Questo package (Figura~\ref{fig:business_logic_strategy}) implementa lo \hyperref[sec:strategy_pattern]{Strategy Pattern} per l'elaborazione dei dati. Definisce:
\begin{itemize}
	...
	\item Una StrategyFactory che gestisce la creazione e l'esecuzione della strategia appropriata.
\end{itemize}
\begin{figure}[h]
	\centering
...
Il package Controller (Figura~\ref{fig:controller}) implementa la parte C del pattern \hyperref[sec:mvc_pattern]{MVC}. Ad ogni vista principale è associato un controller (es. FornitoreController, PiantagioneController) che gestisce gli eventi della GUI (es. onNuovoFornitore()) e invoca i metodi del BusinessLogic layer (tramite i suoi Service) per eseguire le azioni richieste (implementazione del pattern \hyperref[sec:observer_pattern]{Observer}).
\begin{figure}[h]
	\centering
...
\end{figure}
\newpage
\subsubsection{View}\label{sec:view}
Il package View (Figura~\ref{fig:view}) contiene tutte le classi JavaFX che compongono l'interfaccia utente. È suddiviso logicamente in Main Views (le schermate principali come \hyperref[fig:mockup_dashboard]{DashboardView} e \hyperref[fig:mockup_fornitori]{FornitoreView}), Dialogs (le finestre modali per l'inserimento/modifica, es. \hyperref[fig:mockup_dialog_nuovo_fornitore]{FornitoreDialog}, \hyperref[fig:mockup_piantagioni]{PiantagioneDialog}) e Helpers (classi di utilità come NotificationHelper).
\begin{figure}[h]
	\centering
...
\end{figure}
\newpage
\subsection{Progettazione Database}\label{sec:progettazione_database}
La progettazione del database deriva direttamente dalle entità identificate nel \hyperref[fig:domain_model]{DomainModel}. La persistenza dei dati è gestita da un database relazionale PostgreSQL, implementata tramite il \hyperref[sec:dao_pattern]{DAO Pattern}. La Figura~\ref{fig:er_diagram} mostra lo schema Entità-Relazione (E-R) del database, che evidenzia le tabelle principali e le loro associazioni.
\begin{figure}[h]
	\centering
