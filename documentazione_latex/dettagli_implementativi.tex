...
\begin{itemize}
	\item \textbf{Data Access Layer:} Vengono create le istanze dei DAO tramite la DAOFactory.
	\item \textbf{Service Layer:} Vengono istanziati i Service, iniettando i DAO necessari (es. new ZonaService(DAOFactory.getZonaDAO())).
	\item \textbf{Presentation Layer:} Vengono istanziati i Controller, iniettando i Service e le View corrispondenti (es. new ZonaController(zonaService, zonaView)).
\end{itemize}
\subsubsection{Service Layer- Validazione e Business Logic}
...
\begin{figure}[h]
	...
	\caption{Architettura Model-View-Controller (MVC) dell'entità fornitore.}
...
\lstinputlisting[
	...
	firstline=39,
	lastline=50,
	caption={Snippet dell'inizializzazione dei componenti MVC in MainApp.},
...
La classe \texttt{BusinessLogic} (Listing \ref{lst:business_logic}) utilizza il Factory Pattern per selezionare e eseguire la strategia di elaborazione dati appropriata in base al tipo di analisi richiesto, garantendo un'architettura flessibile e facilmente estendibile. L'accesso ai DAO avviene tramite la DAOFactory, che incapsula la logica di creazione e configurazione dei DAO.
\lstinputlisting[
	language=Java,
	firstline=1,
	lastline=24,
...
Il Singleton Pattern è stato adottato per garantire che alcune classi abbiano una sola istanza globale accessibile in modo controllato. Questo è particolarmente utile per componenti come la connessione al database, DAOFactory e l'ErrorService, dove un'unica istanza è sufficiente e desiderabile per gestire le operazioni di accesso ai dati e la gestione degli errori.
\lstinputlisting[
	language=Java,
	firstline=1,
	lastline=27,
...
\subsection{Gestione Errori e Exception Hierarchy}
AgroManager implementa una gerarchia di eccezioni personalizzate (già introdotta visivamente nella Figura \ref{fig:business_logic_exception} a pagina \pageref{fig:business_logic_exception}). Ogni eccezione personalizzata AgroManagerException trasporta tre informazioni fondamentali:
\begin{itemize}
...
Il vantaggio principale di questa architettura emerge nei \texttt{Controller}.
